...
RESULTS_DIR="$BASE_DIR/results_multi_sta_single_priority"
SIM_TIME="12"
PYTHON_BIN="python3"
NUM_STAS_LIST="${NUM_STAS_LIST:-4}"
STA_PRIORITIES="${STA_PRIORITIES:-3,2,1,0}"
...
PROTOS_LIST="${PROTOS_LIST:-UDP TCP}"
...
for DATA_RATE in "${DATA_RATES[@]}"; do
    ...
    SCENARIO_TAG="${DATA_RATE}_${NSTAS}stas"
    ...
    echo ""
    echo "=============================================="
    echo "Running Single Link Multi-STA PRIORITY experiments for Data Rate: $DATA_RATE (per STA)"
    echo "Total potential throughput: ${NSTAS} x $DATA_RATE"
    echo "STA priority classes: $STA_PRIORITIES (0=BK,1=BE,2=VI,3=VO)"
    echo "=============================================="
    ...
    for freq in "${FREQS[@]}"; do
        for proto in "${PROTOS[@]}"; do
            ...
            ./ns3 run "scratch/wifi7-single-link-multi-sta --freq=$freq --protocol=$proto --dataRate=$DATA_RATE --simTime=$SIM_TIME --staticSetup=true --numStas=$NSTAS --staPriorities=$STA_PRIORITIES" 2>&1 | tee "$OUTFILE"
    ...
    if [ "$GENERATE_PYTHON_REPORTS" = "true" ]; then
        ...
        "$PYTHON_BIN" "$BASE_DIR/generate_single_link_loss_breakdown_plots.py" "$CSV_SINGLE_LOSS" "$PLOTS_DIR" "$SCENARIO_TAG" "$CSV_SINGLE_PHY_REASON"
        "$PYTHON_BIN" "$BASE_DIR/generate_multi_sta_phy_drop_reason_table.py" "$CSV_SINGLE_PHY_REASON" "$PLOTS_DIR" "$SCENARIO_TAG"
        "$PYTHON_BIN" "$BASE_DIR/generate_multi_sta_tc_drop_reason_table.py" "$CSV_SINGLE_TC_REASON" "$OUTPUTS_DIR/__mlo_missing__.csv" "$PLOTS_DIR" "$SCENARIO_TAG"
...
PY
        ...
        {
            echo "=============================================================="
            echo "MLO Link/RU Usage Summary - $SCENARIO_TAG"
            echo "=============================================================="
    ...
    echo ""
    echo "=============================================="
    echo "Summary for Data Rate: $DATA_RATE (per STA), STAs=$NSTAS"
    echo "=============================================="
    echo ""
