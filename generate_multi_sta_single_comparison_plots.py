#!/usr/bin/env python3
"""
Generate comparison plots for Multi-STA Single Link experiments across multiple data rates.
Each plot shows groups (data rates) with 3 bars each (frequency bands).
...
def load_all_data():
    ...
    for outputs_dir in output_dirs:
        ...
        # Find the most recent single_multi_sta_results CSV
        csv_files = glob.glob(os.path.join(outputs_dir, "single_multi_sta_results_*.csv"))
        if not csv_files:
            print(f"Warning: No single CSV found in {outputs_dir}")
            continue
...
def create_comparison_plot(df, metric, ylabel, title_suffix, filename_suffix):
    ...
    for proto in protocols:
        ...
        ax.legend(title='Frequency Band', fontsize=14, title_fontsize=14)
        ax.grid(axis='y', alpha=0.3)
        ax.set_title(f'Multi-STA Single Link - {proto}', fontsize=18, fontweight='bold')
        
        plt.tight_layout()
        output_file = os.path.join(OUTPUT_DIR, f'multi_sta_single_comparison_{filename_suffix}_{proto.lower()}.png')
...
def main():
    print("=" * 50)
    print("Generating Multi-STA Single Link Comparison Plots")
    print("=" * 50)
