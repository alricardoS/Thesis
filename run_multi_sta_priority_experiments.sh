...
echo "=============================================="
./ns3 build scratch/wifi7-single-link-multi-sta-priority
./ns3 build scratch/wifi7-mlo-multi-sta-priority
...
for DATA_RATE in "${DATA_RATES[@]}"; do
    ...
    for freq in "${FREQS[@]}"; do
        for proto in "${PROTOS[@]}"; do
            ...
            ./ns3 run "scratch/wifi7-single-link-multi-sta-priority --freq=$freq --protocol=$proto --dataRate=$DATA_RATE --simTime=$SIM_TIME --staticSetup=true --numStas=$NSTAS --staTrafficTypes=$STA_TRAFFIC_TYPES --queueOccupancyCsv=$CSV_SINGLE_QUEUE_OCC$QUEUE_LABEL_ARG --queueSampleInterval=0.1" 2>&1 | tee "$OUTFILE"
    ...
    if [ "$GENERATE_PYTHON_REPORTS" = "true" ]; then
        ...
        "$PYTHON_BIN" "$BASE_DIR/generate_multi_sta_loss_breakdown_plots.py" "$CSV_SINGLE_LOSS" "$CSV_MLO_LOSS" "$PLOTS_DIR" "$SCENARIO_TAG" "$CSV_SINGLE_PHY_REASON" "$CSV_MLO_PHY_REASON" "$CSV_MLO_LINK_TRAFFIC"
