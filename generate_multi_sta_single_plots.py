#!/usr/bin/env python3
"""
generate_multi_sta_single_plots.py
Gera gráficos para testes Single Link com múltiplas STAs
...
if len(sys.argv) < 3:
    sys.exit("Usage: python3 generate_multi_sta_single_plots.py <csv> <out_dir> [data_rate]")
...
if 'loss_rate_pct' in df.columns:
    ...
    ax.set_title(f'WiFi 7 Single Link Packet Loss Rate (802.11be) - Data Rate: {data_rate}')
    ax.legend()
    plt.tight_layout()
...
if 'avg_throughput_per_sta_mbps' in df.columns:
    ...
    ax.set_title(f'WiFi 7 Single Link Avg Throughput per STA (802.11be) - Data Rate: {data_rate}')
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, 'single_link_avg_throughput_per_sta.png'))
    print('Avg Throughput per STA Plot generated:', os.path.join(out_dir, 'single_link_avg_throughput_per_sta.png'))
    plt.close()
