...
- **Mechanism** (in `UpdatePeriodicMetrics`, one move per window):
  ...
  5. **Headroom test (fill up to real capacity):** an **empty** target always accepts the **1st flow** (an idle link's capacity is not measurable, so no test is applied); once the target already carries traffic, `headroom ≥ targetThroughput` is required using the **measured** capacity. The measured capacity then naturally limits the fill — 2.4 GHz holds 1 VI, 5 GHz holds 2, etc. *(An earlier attempt used a per-band nominal capacity for idle targets, but the 2.4 GHz nominal (150 Mbps) equalled the VI demand (150), turning the test into a knife-edge that any residual load — e.g. beacons — flipped into a rejection; that nominal was removed.)*
