/*
 * wifi7-single-link-2ul-2dl.cc
 * 802.11be Single Link Test with 2 UL STAs and 2 DL STAs
 *
 * Layout lógico:
 *    UL_STAs -> AP -> DL_STAs
 *
...
NS_LOG_COMPONENT_DEFINE("wifi7-single-link-2ul-2dl");
...
{
    int freq = 2; // 2, 5, or 6
    std::string dataRateStr = "300Mbps"; // Per-link data rate
    double simTime = 12.0;
    std::string protocol = "UDP";
    double staDistance = STA_DISTANCE;
    bool useStaticSetup = false; // Usar WifiStaticSetupHelper

    bool enableOfdma = true; // Enable OFDMA (DL+UL)
    bool enableUlOfdma = true;
    bool enableBsrp = false;
    uint32_t numUlStas = 2;
    uint32_t numDlStas = 2;
    ...
    cmd.AddValue("enableOfdma", "Enable OFDMA (DL and UL Multi-User)", enableOfdma);
    cmd.AddValue("enableUlOfdma", "Enable UL OFDMA on the AP multi-user scheduler", enableUlOfdma);
    cmd.AddValue("enableBsrp", "Enable BSRP trigger frames on the AP multi-user scheduler", enableBsrp);
    cmd.AddValue("numUlStas", "Number of uplink STAs", numUlStas);
    cmd.AddValue("numDlStas", "Number of downlink STAs", numDlStas);
    cmd.Parse(argc, argv);

    // Set global numStas and initialize vectors
    g_numStas = numUlStas + numDlStas;
    ...
    NS_LOG_UNCOND("wifi7-single-link-2ul-2dl: freq=" << freq << " proto=" << protocol 
                  << " rate=" << dataRateStr << " numULSTAs=" << numUlStas << " numDLSTAs=" << numDlStas
                  << " distance=" << staDistance << "m"
                  << " staticSetup=" << (useStaticSetup ? "yes" : "no")
                  << " OFDMA=" << (enableOfdma ? "yes" : "no"));
    ...
    // Criar nós: 1 AP + STAs UL + STAs DL
    NodeContainer apNode;
    apNode.Create(1);
    NodeContainer ulStaNodes;
    ulStaNodes.Create(numUlStas);
    NodeContainer dlStaNodes;
    dlStaNodes.Create(numDlStas);

    NodeContainer allStaNodes;
    allStaNodes.Add(ulStaNodes);
    allStaNodes.Add(dlStaNodes);
    NodeContainer staNodes = allStaNodes;
    ...
    // Configure the Multi-User Scheduler unconditionally so DL OFDMA stays available.
    // The toggles below only affect UL OFDMA and BSRP behavior.
    ...
    NS_LOG_UNCOND("DL-OFDMA scheduler enabled; enableOfdma=" << (enableOfdma ? "yes" : "no")
                                                              << " UL-OFDMA=" << (enableUlOfdma ? "yes" : "no")
                                                              << " BSRP=" << (enableBsrp ? "yes" : "no"));
    ...
    NetDeviceContainer ulStaDev = wifi.Install(phy, staMac, ulStaNodes);
    NetDeviceContainer dlStaDev = wifi.Install(phy, staMac, dlStaNodes);
   ...
   if (useStaticSetup) {
    ...
    // Uplink traffic: STAs -> AP
    for (uint32_t i = 0; i < numUlStas; ++i)
    {
        uint16_t port = basePort + sinkIndex;

        Address sinkAddr(InetSocketAddress(ifaces.GetAddress(0), port));
        ...
        sinks[sinkIndex] = DynamicCast<PacketSink>(sinkApp.Get(0));
        sinks[sinkIndex]->TraceConnectWithoutContext("Rx", MakeBoundCallback(&MonitorPacketSinkRx, sinkIndex));
        ...
        ApplicationContainer clientApp = client.Install(ulStaNodes.Get(i));
        clientApp.Start(APP_START_TIME + Seconds(1));
        clientApp.Stop(SIMULATION_END_TIME - Seconds(2));

        NS_LOG_UNCOND("App configured: UL STA" << i << " -> AP (port " << port << ")");
        sinkIndex++;
    }

    // Downlink traffic: AP -> STAs
    for (uint32_t i = 0; i < numDlStas; ++i)
    {
        uint16_t port = basePort + sinkIndex;
        uint32_t staNodeIndex = numUlStas + i;

        Address sinkAddr(InetSocketAddress(ifaces.GetAddress(1 + staNodeIndex), port));
        PacketSinkHelper sinkHelper(appSocketType, sinkAddr);
        ApplicationContainer sinkApp = sinkHelper.Install(dlStaNodes.Get(i));
        sinkApp.Start(APP_START_TIME);
        sinkApp.Stop(SIMULATION_END_TIME - Seconds(1));
        sinks[sinkIndex] = DynamicCast<PacketSink>(sinkApp.Get(0));
        sinks[sinkIndex]->TraceConnectWithoutContext("Rx", MakeBoundCallback(&MonitorPacketSinkRx, sinkIndex));
        ...
        clientApp.Start(APP_START_TIME + Seconds(2));
        clientApp.Stop(SIMULATION_END_TIME - Seconds(3));

        NS_LOG_UNCOND("App configured: AP -> DL STA" << i << " (port " << port << ")");
        sinkIndex++;
    }
    ...
    PrintNodes(ulStaNodes, "UL-STA");
    PrintNodes(dlStaNodes, "DL-STA");
    ...
    for (const auto &kv : stats) {
        ...
        bool isDownlink = (t.sourceAddress == apAddr);
        bool isUplink = (t.destinationAddress == apAddr);

        if (isDownlink || isUplink) {
            ...
            for (uint32_t i = 0; i < numUlStas; ++i)
            {
                if (isUplink && t.sourceAddress == ifaces.GetAddress(1 + i))
                {
                    NS_LOG_UNCOND("FLOW_SUMMARY_UL_STA" << i << ": freq=" << freq << " proto=" << protocol
                                  << " Throughput_Mbps=" << tp
                                  << " Delay_ms=" << delayMs
                                  << " Jitter_ms=" << jitterMs
            ...
            {
                uint32_t staNodeIndex = numUlStas + i;
                if (isDownlink && t.destinationAddress == ifaces.GetAddress(1 + staNodeIndex))
                {
                    NS_LOG_UNCOND("FLOW_SUMMARY_DL_STA" << i << ": freq=" << freq << " proto=" << protocol
                                  << " Throughput_Mbps=" << tp
                                  << " Delay_ms=" << delayMs
                                  << " Jitter_ms=" << jitterMs
                                  << " LostPackets=" << lostPackets
    ...
    double avgJitter = (validFlows > 0) ? totalJitter / validFlows : 0.0;
    double avgThroughputPerSta = (validFlows > 0) ? totalThroughput / validFlows : 0.0;
    double totalLossRate = (totalTxPackets > 0) ? (totalLostPackets * 100.0 / totalTxPackets) : 0.0;
    
    NS_LOG_UNCOND("FLOW_SUMMARY_TOTAL: freq=" << freq << " proto=" << protocol << " numFlows=" << validFlows
