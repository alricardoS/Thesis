...
int main(int argc, char* argv[])
{
    ...
    int freq1 = 5;
    int freq2 = 6;
    std::string dataRateStr = "500Mbps"; // Per-STA (reduzido para evitar saturação com 4 STAs)
    double simTime = 12.0;
    ...
    std::string staTrafficTypesCsv = "best-effort,voice,video,video";
    ...
    g_linkFrequencyPair = std::to_string(freq1) + "+" + std::to_string(freq2);
    ...
    g_staLinkMetricsRunLabel = staLinkMetricsLabel.empty()
                                   ? (queueOccupancyLabel.empty() ? (std::to_string(freq1) + "+" + std::to_string(freq2) + "|" + protocol)
    ...
    if (!g_staLinkMetricsCsvPath.empty())
    {
        ...
        g_staLinkMetricsEndTime = Seconds(simTime);
    }
    ...
    {
        g_apRxAddresses.insert(apWifiNetDev->GetMac()->GetFrameExchangeManager(linkId)->GetAddress());
    }
    ...
    for (uint32_t i = 0; i < g_numStas; ++i) {
        ...
        app.Get(0)->TraceConnectWithoutContext("TxWithSeqTsSize", MakeBoundCallback(&MonitorAppTx, i));
