#!/usr/bin/env python3
"""
Generate comparison plots for Single Link experiments across multiple data rates.
Each plot shows 4 groups (data rates) with 3 bars each (frequency bands).
...
OUTPUT_DIR = os.path.join(RESULTS_DIR, "comparison_plots_single")
...
# Frequency bands for single link
FREQ_BANDS = ["2.4GHz", "5GHz", "6GHz"]
...
def load_all_data():
    ...
    for dr in DATA_RATES:
        ...
        # Find the most recent single_results CSV
        csv_files = glob.glob(os.path.join(outputs_dir, "single_results_*.csv"))
        if not csv_files:
            print(f"Warning: No single_results CSV found in {outputs_dir}")
            continue
...
def create_comparison_plot(df, metric, ylabel, title_suffix, filename_suffix):
    ...
    for proto in protocols:
        ...
        for i, freq in enumerate(FREQ_BANDS):
            ...
            bars = ax.bar(x + offset, vals, width=width, label=freq, 
                         color=COLORS[freq], hatch=HATCHES[freq], edgecolor='black', linewidth=0.5)
            # Use appropriate format: no decimals for throughput, 2 decimals for jitter/delay
        ...
        ax.legend(title='Frequency Band', fontsize=17, title_fontsize=16)
        ax.grid(axis='y', alpha=0.3)
        
        plt.tight_layout()
        output_file = os.path.join(OUTPUT_DIR, f'single_comparison_{filename_suffix}_{proto.lower()}.png')
...
def main():
    print("=" * 50)
    print("Generating Single Link Comparison Plots")
    print("=" * 50)
    
    df = load_all_data()
    print(f"\nLoaded {len(df)} rows of data")
    print(f"Data rates found: {df['data_rate'].unique()}")
    print(f"Frequency bands found: {df['freq_band'].unique()}")
