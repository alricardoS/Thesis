...
RESULTS_ROOT_DIR="${RESULTS_ROOT_DIR:-$BASE_DIR/results_multi_sta_priority_suite_base}"
...
# ==============================
EXPERIMENTS=(
  "all_voice|voice,voice,voice,voice"
  "all_video|video,video,video,video"
  "all_besteffort|besteffort,besteffort,besteffort,besteffort"
)
...
for item in "${EXPERIMENTS[@]}"; do
  ...
  # After runner completes, aggregate per-AC and summed metrics into suite-level folders
  if command -v python3 >/dev/null 2>&1; then
    python3 "$BASE_DIR/scripts/aggregate_multi_sta_priority.py" --results-root "$RESULTS_ROOT_DIR" || echo "[WARN] Aggregation script failed for $EXP_NAME"
  else
    echo "[WARN] python3 not found; skipping aggregation"
  fi
