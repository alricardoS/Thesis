...
RESULTS_DIR="${RESULTS_DIR:-$BASE_DIR/results_multi_sta_priority}"
SIM_TIME="17"
PYTHON_BIN="python3"
...
DATA_RATES_LIST="${DATA_RATES_LIST:-30Mbps 150Mbps 250Mbps}"
...
./ns3 build scratch/wifi7-single-link-multi-sta-priority-per-sta
./ns3 build scratch/wifi7-mlo-multi-sta-priority-per-sta
...
for DATA_RATE in "${DATA_RATES[@]}"; do
    ...
    echo ""
    echo "=============================================="
    echo "Running Multi-STA PRIORITY experiments for Data Rate: $DATA_RATE (per STA) $OFDMA_LABEL"
    ...
    CSV_SINGLE_QUEUE_OCC="$OUTPUTS_DIR/single_multi_sta_queue_occupancy.csv"
    echo "run_label,time_s,role,node_id,ac,packets,bytes" > "$CSV_SINGLE_QUEUE_OCC"

    CSV_SINGLE_QUEUE_OCC_PER_STA="$OUTPUTS_DIR/single_multi_sta_queue_occupancy_per_sta.csv"
    echo "run_label,time_s,sta_id,ac,tx_packets,tx_bytes,rx_packets,rx_bytes,backlog_packets,backlog_bytes,avg_delay_ms,avg_jitter_ms" > "$CSV_SINGLE_QUEUE_OCC_PER_STA"
    ...
    for freq in "${FREQS[@]}"; do
        for proto in "${PROTOS[@]}"; do
            ...
            ./ns3 run "scratch/wifi7-single-link-multi-sta-priority-per-sta --freq=$freq --protocol=$proto --dataRate=$DATA_RATE --simTime=$SIM_TIME --staticSetup=true --numStas=$NSTAS --staTrafficTypes=$STA_TRAFFIC_TYPES --queueOccupancyCsv=$CSV_SINGLE_QUEUE_OCC$QUEUE_LABEL_ARG --staLinkMetricsCsv=$CSV_SINGLE_QUEUE_OCC_PER_STA$QUEUE_LABEL_ARG --staLinkMetricsSampleInterval=0.1 --queueSampleInterval=0.1" 2>&1 | tee "$OUTFILE"
    ...
    CSV_MLO_QUEUE_OCC_PER_STA="$OUTPUTS_DIR/mlo_multi_sta_queue_occupancy_per_sta.csv"
    echo "run_label,time_s,sta_id,ac,tx_packets,tx_bytes,rx_packets,rx_bytes,backlog_packets,backlog_bytes,avg_delay_ms,avg_jitter_ms" > "$CSV_MLO_QUEUE_OCC_PER_STA"
    ...
    for pair in "${PAIRS[@]}"; do
        ...
        for proto in "${PROTOS[@]}"; do
            ...
            ./ns3 run "scratch/wifi7-mlo-multi-sta-priority-per-sta --freq1=$F1 --freq2=$F2 --protocol=$proto --dataRate=$DATA_RATE --simTime=$SIM_TIME --staticSetup=true --enablePcaps=false --numStas=$NSTAS --staTrafficTypes=$STA_TRAFFIC_TYPES --queueOccupancyCsv=$CSV_MLO_QUEUE_OCC$QUEUE_LABEL_ARG --staLinkMetricsCsv=$CSV_MLO_QUEUE_OCC_PER_STA$QUEUE_LABEL_ARG --staLinkMetricsSampleInterval=0.1 --queueSampleInterval=0.1" 2>&1 | tee "$OUTFILE"
    ...
    if [ "$GENERATE_PYTHON_REPORTS" = "true" ]; then
        ...
        "$PYTHON_BIN" "$BASE_DIR/generate_multi_sta_loss_breakdown_plots.py" "$CSV_SINGLE_LOSS" "$CSV_MLO_LOSS" "$PLOTS_DIR" "$SCENARIO_TAG" "$CSV_SINGLE_PHY_REASON" "$CSV_MLO_PHY_REASON"
        ...
        "$PYTHON_BIN" "$BASE_DIR/generate_multi_sta_phy_drop_reason_table.py" "$CSV_SINGLE_PHY_REASON" "$PLOTS_DIR" "$SCENARIO_TAG"
        "$PYTHON_BIN" "$BASE_DIR/generate_multi_sta_tc_drop_reason_table.py" "$CSV_SINGLE_TC_REASON" "$CSV_MLO_TC_REASON" "$PLOTS_DIR" "$SCENARIO_TAG"

        "$PYTHON_BIN" - "$CSV_SINGLE_LOSS" "$PLOTS_DIR" "$SCENARIO_TAG" <<'PY'
import os
import sys
import pandas as pd
...
PY
        ...
        "$PYTHON_BIN" - "$CSV_MLO" "$CSV_MLO_LOSS" "$PLOTS_DIR" "$SCENARIO_TAG" <<'PY'
import os
import sys
import pandas as pd

results_csv, loss_csv, out_dir, scenario = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
...
freq_parts = merged["pair"].apply(split_pair)
merged["freq_1_gHz"] = freq_parts.apply(lambda x: x[0])
merged["freq_2_gHz"] = freq_parts.apply(lambda x: x[1])
...
merged = merged[ordered_cols].sort_values(["pair", "protocol"])
...
PY
        ...
        "$PYTHON_BIN" - "$CSV_SINGLE_PHY_PKT_TYPE" "$PLOTS_DIR" "$SCENARIO_TAG" <<'PY'
import os
import sys
import pandas as pd
...
PY
        ...
        "$PYTHON_BIN" - "$CSV_SINGLE_PHY_PKT_SIZE" "$PLOTS_DIR" "$SCENARIO_TAG" <<'PY'
import os
import sys
import pandas as pd
...
PY
        ...
        if [ -f "$CSV_SINGLE_QUEUE_OCC" ]; then
            "$PYTHON_BIN" "$BASE_DIR/generate_queue_occupancy_plots.py" "$CSV_SINGLE_QUEUE_OCC" "$PLOTS_DIR/queue_occupancy_single" "Single-Link Queue Occupancy"
        fi
        ...
        if [ -f "$CSV_SINGLE_QUEUE_OCC_PER_STA" ]; then
            "$PYTHON_BIN" "$BASE_DIR/generate_per_sta_queue_plots.py" "$CSV_SINGLE_QUEUE_OCC_PER_STA" "$PLOTS_DIR/queue_occupancy_single_per_sta" "Single-Link Per-STA Queue Occupancy"
        fi
        if [ -f "$CSV_MLO_QUEUE_OCC_PER_STA" ]; then
            "$PYTHON_BIN" "$BASE_DIR/generate_per_sta_queue_plots.py" "$CSV_MLO_QUEUE_OCC_PER_STA" "$PLOTS_DIR/queue_occupancy_mlo_per_sta" "MLO Per-STA Queue Occupancy"
        fi
...
echo "Comparison plots in: $RESULTS_DIR/comparison_plots_single/ and $RESULTS_DIR/comparison_plots_mlo/"
